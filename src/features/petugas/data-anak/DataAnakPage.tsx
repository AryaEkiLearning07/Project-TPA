...
import { downloadDataAnakRegistrationPdf } from './dataAnakRegistrationPdf'
...
const createInitialForm = (): ChildProfileInput => ({
  ...
  serviceStartDate: getLocalDateIso(),
  arrivalTime: '',
  departureTime: '',
  pickupPersons: [],
  depositPurpose: '',
...
const DataAnakPage = ({
  ...
  const [isDownloadingDetailPdf, setDownloadingDetailPdf] = useState(false)
  const [detailPdfNotice, setDetailPdfNotice] = useState<string | null>(null)
  ...
  const handleEdit = (record: ChildProfile, e: React.MouseEvent) => {
    ...
    setForm({
      ...createInitialForm(),
      ...input,
      serviceStartDate: input.serviceStartDate || getLocalDateIso(),
    })
    setErrors({})
    setEditingId(id)
    setPickupInput('')
  ...
  const handleSelectChild = (child: ChildProfile) => {
    ...
    setDetailPdfNotice(null)
    void loadRegistrationCodeForChild(child.id)
  }

  const handleDownloadDetailPdf = async () => {
    if (!selectedChild || viewerRole !== 'ADMIN' || isDownloadingDetailPdf) {
      return
    }

    setDetailPdfNotice(null)
    setDownloadingDetailPdf(true)
    try {
      const result = await downloadDataAnakRegistrationPdf({
        child: selectedChild,
      })
      setDetailPdfNotice(
        result.logoLoaded
          ? `PDF berhasil diunduh (${result.pageCount} halaman).`
          : `PDF berhasil diunduh (${result.pageCount} halaman), tetapi logo tidak ditemukan.`,
      )
    } catch {
      setDetailPdfNotice('Gagal membuat PDF detail anak. Silakan coba lagi.')
    } finally {
      setDownloadingDetailPdf(false)
    }
  }
      ...
      <section className="page">
          ...
          <div className="form-actions" style={{ marginBottom: '1rem' }}>
            ...
            {viewerRole === 'ADMIN' ? (
              <button
                ...
                onClick={() => void handleDownloadDetailPdf()}
                disabled={isDownloadingDetailPdf}
              >
                {isDownloadingDetailPdf ? 'Membuat PDF...' : 'Unduh PDF'}
              </button>
            ) : null}
          </div>
          {detailPdfNotice ? (
            <p className="field-hint" style={{ marginTop: '-0.4rem', marginBottom: '0.9rem' }}>
              {detailPdfNotice}
            </p>
          ) : null}
        ...
        <div className="card">
          ...
          <div className="detail-grid">
            ...
            <div className="detail-item"><span className="detail-label">Tanggal Mulai Masuk</span><span className="detail-value">{formatDate(child.serviceStartDate)}</span></div>
              ...
              <div key={child.id} className="ktp-card" onClick={() => handleSelectChild(child)}>
                ...
                <div className="ktp-card__bottom">
                  <div className="ktp-card__bottom-left">
                    {child.allergy ? (
                      <span className="ktp-card__allergy">
                        <span className="ktp-card__allergy-icon" aria-hidden="true">
                          <svg viewBox="0 0 24 24" width="13" height="13" fill="none">
                            <path d="M12 3L22 21H2L12 3Z" fill="currentColor" />
                            <path d="M12 9V14.5" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" />
                            <circle cx="12" cy="18" r="1.2" fill="#ffffff" />
                          </svg>
                        </span>
                        <span className="ktp-card__allergy-text">{child.allergy}</span>
                      </span>
                    ) : (
          ...
          <div className="modal-content modal-content--large">
            ...
            <form onSubmit={handleSubmit} className="modal-body">
              ...
              <div className="form-grid form-grid--3">
                ...
                <div className="field-group">
                  <label className="label" htmlFor="serviceStartDate">Tanggal mulai masuk</label>
                  <AppDatePickerField
                    id="serviceStartDate"
                    value={form.serviceStartDate}
                    onChange={(value) => setField('serviceStartDate', value)}
                  />
                  {errors.serviceStartDate ? <p className="field-error">{errors.serviceStartDate}</p> : null}
                </div>
                <div className="field-group">
